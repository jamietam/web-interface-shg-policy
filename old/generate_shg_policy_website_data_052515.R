## Create raw data files for SHG policy module web interface
setwd( "C:/Users/Jamie/Dropbox/CISNET/Policy_Module/website" )
library(reshape)
library(data.table)
...
prevalencesF <- prevalencesM[order(prevalencesM$year,prevalencesM$age),]# Sort by year, age, policy
...
# baselineM <- baselineM[keepvars]
# baselineF <- baselineF[keepvars]
# policyM <- policyM[keepvars]
# policyF <- policyF[keepvars]

# setnames(baselineM, "smoking_prevalence", "males_baseline")
# setnames(baselineF, "smoking_prevalence", "females_baseline")
# setnames(policyM, "smoking_prevalence", "males_policy")
# setnames(policyF, "smoking_prevalence", "females_policy")

# finalprevalencedata <- merge(baselineM,baselineF,by=c("year","age","cohort"))
# finalprevalencedata <- merge(finalprevalencedata,policyM,by=c("year","age","cohort"))
# finalprevalencedata <- merge(finalprevalencedata,policyF,by=c("year","age","cohort"))
# finalprevalencedata <- finalprevalencedata[order(finalprevalencedata$year,finalprevalencedata$age,finalprevalencedata$cohort),]

# Aggregate for age groups ------------------------------------------------
# Reshape the dataframe to the same format as census file -----------------

policymoduledata <- c(baselineM, baselineF,policyM,policyF)

for (dataframe in policymoduledata){
  dataframe <- melt(dataframe, id.vars=c("age","year"),measure.vars="smoking_prevalence") # Baseline scenario, males
  dataframe <- cast(dataframe, age ~ year)
  dataframe <- subset(dataframe, select=cbind("2010","2011","2012","2013","2014","2015","2016","2017","2018","2019","2020","2021","2022","2023","2024","2025","2026","2027","2028","2029",
                                              "2030","2031","2032","2033","2034","2035","2036","2037","2038","2039","2040","2041","2042","2043","2044","2045","2046","2047","2048","2049",
                                              "2050","2051","2052","2053","2054","2055","2056","2057","2058","2059","2060"))
  rownames(dataframe) <- ages
  colnames(dataframe) <- years
...
smokerprevs <- function(age,year,smokerpopulation,totalpopulation){ ## Ages 0-99, Years 2010-2060
  smkrprev <- smokerpopulation[age+1,year-2009]/totalpopulation[age+1,year-2009]
  return(smkrprev)
}

smokerprevs(99,2060,smokpopbaseM,popmales)
...
smokprevbase <- smokpopbaseM+smokpopbaseF/colSums(populationgroup,na.rm=TRUE) # Total population
...
prevtable <- cbind(prevbase, prevbaseM, prevbaseF, prevpolicy, prevpolicyM, prevpolicyF)
colnames(prevtable) <- c("Baseline scenario", "Baseline - Males", "Baseline - Females", "Policy scenario", "Policy - Men", "Policy - Women")
