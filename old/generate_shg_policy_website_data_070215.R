...
smokerprevs <- function(age,year,smokpopbaseM,smokpoppolicyM,smokpopbaseF,smokpoppolicyF,population){ ## Ages 0-99, Years 2010-2060
  ...
  return(c(baseline_smkprev*100, policy_smkprev*100))
}
...
getcumulativedeaths <- function(dataframe,specifyyear){
  ...
  deaths <- colSums(theseyearsonly[c("tobaccodeathsM_baseline", "tobaccodeathsM_policy", "deathsM_avoided","tobaccodeathsF_baseline","tobaccodeathsF_policy","deathsF_avoided","tobaccodeaths_baseline","tobaccodeaths_policy","deaths_avoided")], na.rm = TRUE)
  return(deaths)
}
...
createresultsfile <- function(population, popmales, popfemales, prevalencesM, prevalencesF, policy_year,agegroups,agegroupstart, agegroupend){
  ...
  for(row in 1:length(finalprevs$age)) { 
    finalprevs$totalprev_baseline[row] <- round(smokerprevs(finalprevs$age[row], finalprevs$year[row],smokpopbaseM,smokpoppolicyM,smokpopbaseF,smokpoppolicyF,population)[1],0)
    finalprevs$totalprev_policy[row] <- round(smokerprevs(finalprevs$age[row], finalprevs$year[row],smokpopbaseM,smokpoppolicyM,smokpopbaseF,smokpoppolicyF,population)[2],0)    
  }
  finalprevs$males_baseline <- round(finalprevs$males_baseline*100,0)
  finalprevs$females_baseline	<- round(finalprevs$females_baseline*100,0)
  finalprevs$males_policy	<- round(finalprevs$males_policy*100,0)
  finalprevs$females_policy <- round(finalprevs$females_policy*100,0)
  ...
  for (x in 1:length(agegroups)){
    ...
    thisagegroup <- cbind(c(startingyear:endingyear),agegroups[x], "ALL", round(prevbaseM*100,0), round(prevbaseM_former*100,0), round(prevbaseF*100,0), round(prevbaseF_former*100,0),round(prevpolicyM*100,0), round(prevpolicyM_former*100,0),round(prevpolicyF*100,0),round(prevpolicyF_former*100,0), round(prevbase*100,0), round(prevpolicy*100,0))
  ...
  limitvars <- c("year","age","cohort","males_baseline","females_baseline","males_policy","females_policy","totalprev_baseline","totalprev_policy")
  ...
  for (y in startingyear:endingyear){
    ...
    for (a in 1:length(ages)){
      ...
      deathsM_avoided <- tobaccodeathsM_baseline-tobaccodeathsM_policy
      ...
      deathsF_avoided <- tobaccodeathsF_baseline-tobaccodeathsF_policy
      ...
      temp = rbind(temp, c(tobaccodeathsM_baseline,tobaccodeathsM_policy,deathsM_avoided,tobaccodeathsF_baseline,tobaccodeathsF_policy,deathsF_avoided,tobaccodeaths_baseline,tobaccodeaths_policy,deaths_avoided))
    }
    deaths <- colSums(temp)
    deaths_df <- rbind(deaths_df, c(y,deaths))
  }
  names(deaths_df)<- c("year","tobaccodeathsM_baseline", "tobaccodeathsM_policy", "deathsM_avoided", "tobaccodeathsF_baseline", "tobaccodeathsF_policy", "deathsF_avoided",
  ...
  for (r in 1:nrow(deaths_df)){
    ...
    deaths_df$cumulativedeathsM_avoided[r] <- getcumulativedeaths(deaths_df, deaths_df$year[r])[3]
    ...
    deaths_df$cumulativedeathsF_avoided[r] <- getcumulativedeaths(deaths_df, deaths_df$year[r])[6]
...
colnames(finalprevs) <- c("year","age","cohort","males_baseline","females_baseline","males_policy","females_policy","totalprev_baseline","totalprev_policy", "policy_year")
colnames(deaths_df) <- c("year", "tobaccodeathsM_baseline", "tobaccodeathsM_policy", "deathsM_avoided", 
                         "tobaccodeathsF_baseline", "tobaccodeathsF_policy", "deathsF_avoided",
                         "tobaccodeaths_baseline", "tobaccodeaths_policy", "deaths_avoided" , 
                         "cumulativedeathsM_baseline", "cumulativedeathsM_policy", "cumulativedeathsM_avoided",
                         "cumulativedeathsF_baseline", "cumulativedeathsF_policy", "cumulativedeathsF_avoided", 
                         "cumulativedeaths_baseline", "cumulativedeaths_policy", "cumulativedeaths_avoided", "policy_year" )
