...
# createresultsfile <- function(population, popmales, popfemales, prevalencesM, prevalencesF, policy_year,agegroups,agegroupstart, agegroupend){
  ...
  getdeathcounts <- function(age,year){
    # Generate tobacco-related death counts
    smkrdeathsM_base <- tobaccodeaths_males_current[age+1,year-2009]*smokpopbaseM[age+1,year-2009]
    formerdeathsM_base <- tobaccodeaths_males_former[age+1,year-2009]*formerpopbaseM[age+1,year-2009]
    smkrdeathsM_pol <- tobaccodeaths_males_current[age+1,year-2009]*smokpoppolicyM[age+1,year-2009]
    formerdeathsM_pol <- tobaccodeaths_males_former[age+1,year-2009]*formerpoppolicyM[age+1,year-2009]
    smkrdeathsF_base <- tobaccodeaths_females_current[age+1,year-2009]*smokpopbaseF[age+1,year-2009]
    formerdeathsF_base <- tobaccodeaths_females_former[age+1,year-2009]*formerpopbaseF[age+1,year-2009]
    smkrdeathsF_pol <- tobaccodeaths_females_current[age+1,year-2009]*smokpoppolicyF[age+1,year-2009]
    formerdeathsF_pol <- tobaccodeaths_females_former[age+1,year-2009]*formerpoppolicyF[age+1,year-2009]
    return(c(smkrdeathsM_base,formerdeathsM_base,smkrdeathsM_pol,formerdeathsM_pol,smkrdeathsF_base,formerdeathsF_base,smkrdeathsF_pol,formerdeathsF_pol))
  }
  for(row in 1:length(finalprevs$age)) { 
    ...
    finalprevs$smokerdeaths_baselineM[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[1]
    finalprevs$formerdeaths_baselineM[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[2]
    finalprevs$smokerdeaths_policyM[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[3]
    finalprevs$formerdeaths_policyM[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[4]
    finalprevs$smokerdeaths_baselineF[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[5]
    finalprevs$formerdeaths_baselineF[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[6]
    finalprevs$smokerdeaths_policyF[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[7]
    finalprevs$formerdeaths_policyF[row] <- getdeathcounts(finalprevs$age[row], finalprevs$year[row])[8]
    finalprevs$totaldeaths_baseline[row] <- finalprevs$smokerdeaths_baselineM[row]+finalprevs$formerdeaths_baselineM[row]+finalprevs$smokerdeaths_baselineF[row]+finalprevs$formerdeaths_baselineF[row]
    finalprevs$totaldeaths_baselineM[row] <- finalprevs$smokerdeaths_baselineM[row]+finalprevs$formerdeaths_baselineM[row]
    finalprevs$totaldeaths_baselineF[row] <- finalprevs$smokerdeaths_baselineF[row]+finalprevs$formerdeaths_baselineF[row]
    finalprevs$totaldeaths_policy[row] <- finalprevs$smokerdeaths_policyM[row]+finalprevs$formerdeaths_policyM[row]+finalprevs$smokerdeaths_policyF[row]+finalprevs$formerdeaths_policyF[row]
    finalprevs$totaldeaths_policyM[row] <- finalprevs$smokerdeaths_policyM[row]+finalprevs$formerdeaths_policyM[row]
    finalprevs$totaldeaths_policyF[row] <- finalprevs$smokerdeaths_policyF[row]+finalprevs$formerdeaths_policyF[row]
    finalprevs$totaldeaths_avoided[row] <- finalprevs$totaldeaths_policy[row]-finalprevs$totaldeaths_baseline[row]
    finalprevs$totaldeaths_avoidedM[row] <- finalprevs$totaldeaths_policyM[row]-finalprevs$totaldeaths_baselineM[row]
    finalprevs$totaldeaths_avoidedF[row] <- finalprevs$totaldeaths_policyF[row]-finalprevs$totaldeaths_baselineF[row]
  ...
  for (x in 1:length(agegroups)){
    ...
    thisagegroup <- cbind(c(startingyear:endingyear),agegroups[x], "ALL", prevbaseM, prevbaseF, prevpolicyM, prevpolicyF, prevbase, prevpolicy,prevbaseM_former, prevbaseF_former,prevpolicyM_former,prevpolicyF_former)
    finalprevs_agegroups <- rbind(finalprevs_agegroups,thisagegroup)
    
  }
  names(finalprevs_agegroups) <- c("year","age","cohort","males_baseline","females_baseline","males_policy","females_policy","total_baseline","total_policy", "males_former_baseline","females_former_baseline","males_former_policy","females_former_policy")
  finalprevs_agegroups$..... <- NA
