...
cohortages <- function(birthcohort){
  z <- prevalences$age[(prevalences$cohort==birthcohort)&(prevalences$policy_number==1)] 
  return(z)
}

for (i in 1:2) {
  if (i=1) gender=0 else gender=1 # Males: census sex = 1, prevalences gender = 0
  if (i=1) sex=1 else sex=2 # Females: census sex = 2, prevalences gender = 1
  
  figcohort <- file.path("C:","Users","jamietam", "Dropbox", "CISNET","Policy_module","website","Results",paste("SmokprevCohort_",coverage,"cov_",sexes[i],".pdf",sep=""))
  pdf(file=figcohort)
  titlecohort <- paste("Smoking prevalence by birth-cohort - ", sexes[i])
  count=1 
  par(cex.axis=1.3,cex.lab=1.3,cex.main=1.3,cex.sub=1.3,lwd=2, oma=c(1,1,2,1))
  op <- par(mar = c(5,7,6,2) + 0.1)
  plot(NULL, xlim=c(0,100),ylim=c(0,0.6),ylab='',xlab='Age',axes=FALSE)
  for (birthcohort in selectcohorts){
    lines(cohortages(birthcohort),prevalences$smoking_prevalence[(prevalences$cohort==birthcohort)&(prevalencesM$gender==gender)&(prevalencesM$policy_number==0)],col=count,lty=1)
    lines(cohortages(birthcohort),prevalences$smoking_prevalence[(prevalences$cohort==birthcohort)&(prevalencesM$gender==gender)&(prevalencesM$policy_number==1)],col=count,lty=2)
    count=count+1
  }
  box()
  axis(side=2,las=1)
...
prevalencesM <- read.csv("prevalences_nocoverageMen.csv", header=TRUE) # Read in policy module output data
prevalencesF <- read.csv("prevalences_nocoverageWomen.csv", header=TRUE) 

# Males: census sex = 1, prevalences gender = 0
# Females: census sex = 2, prevalences gender = 1
# Both: census sex = 0

# Specify items for figures ------------------------------
startingyear = 2010
endingyear = 2060
...
baselineM <- men[(men$policy_number==0 & men$year>=startingyear & men$year<=endingyear),] #Split policy module output into baseline and policy scenarios
...
# Baseline scenario -------------------------------------------------------
baselineM <- melt(baselineM, id.vars=c("age","year"),measure.vars="smoking_prevalence") # Reshape the dataframe to the same format as census file
...
baselineF <- melt(baselineF, id.vars=c("age","year"),measure.vars="smoking_prevalence") # Females
...
# Policy scenario ---------------------------------------------------------
policyM <- melt(policyM, id.vars=c("age","year"),measure.vars="smoking_prevalence") # Reshape the dataframe to the same format as census file
...
policyF <- melt(policyF, id.vars=c("age","year"),measure.vars="smoking_prevalence") # Femalse
...
# Age-specific number of male and female smokers ---------------------------------
smokpopbaseM <- matrix(ncol=ncol(popmales),nrow=nrow(popmales)) # Males, baseline
for (x in 1:ncol(popmales)){
  for (y in 1:nrow(popmales)) {
    z <- as.numeric(popmales[y,x])%o% as.numeric(baselineM[y,x])# multiply census population sizes by age-specific smoking prevalences
    smokpopbaseM[y,x] <- z
  }
}
smokpoppolicyM <- matrix(ncol=ncol(popmales),nrow=nrow(popmales)) # Males, Policy
for (x in 1:ncol(popmales)){
  for (y in 1:nrow(popmales)) {
...
smokpopbaseF <- matrix(ncol=ncol(popfemales),nrow=nrow(popfemales)) # Females, baseline
...
smokpoppolicyF <- matrix(ncol=ncol(popmales),nrow=nrow(popmales)) # Females, policy
for (x in 1:ncol(popmales)){
  for (y in 1:nrow(popmales)) {
...
#  ------------------------------------------------------------------------
pdf('SmokprevAnnual_nocoverage.pdf') ### CHECK FILE NAME
