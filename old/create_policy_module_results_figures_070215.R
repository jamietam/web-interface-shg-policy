...
deaths <- read.csv("deaths_w1_r1_b1_w0_r0_b0_2015.csv",header=TRUE)


# Create deaths figures ---------------------------------------------------
pdf('cumulativedeaths_nocov.pdf') ### CHECK FILE NAME
par(cex.axis=1.3,cex.lab=1.3,cex.main=1.3,cex.sub=1.3,lwd=2, oma=c(1,1,2,1))
op <- par(mar = c(5,7,4,2) + 0.1)
plot(NULL, xlim=c(2015,2060),ylim=c(0,2500000),ylab='',xlab='Year',axes=FALSE)
lines(deaths$year,deaths$cumulativedeaths_avoided)
lines(deaths$year,deaths$cumulativedeathsM_avoided,col="blue",lty=1)
lines(deaths$year,deaths$cumulativedeathsF_avoided,col="red",lty=1)
box()
axis(side=2,las=1)
axis(side=1)
title(ylab="Deaths avoided", line=6)
title(main='Cumulative number of deaths avoided', line=2)
legend('topleft',legend=c("Total","Males","Females"),lty=c(1,1,1),col=c("black","blue","red"))
dev.off()

# Create deaths figures ---------------------------------------------------
pdf('deaths_nocov.pdf') 
par(cex.axis=1.3,cex.lab=1.3,cex.main=1.3,cex.sub=1.3,lwd=2, oma=c(1,1,2,1))
op <- par(mar = c(5,7,4,2) + 0.1)
plot(NULL, xlim=c(2015,2060),ylim=c(0,70000),ylab='',xlab='Year',axes=FALSE) ### Display 2015-2065
lines(deaths$year,deaths$deaths_avoided)
lines(deaths$year,deaths$deathsM_avoided,col="blue",lty=1)
lines(deaths$year,deaths$deathsF_avoided,col="red",lty=1)
box()
axis(side=2,las=1)
axis(side=1)
title(ylab="Deaths avoided", line=6)
title(main='Number of deaths avoided by year', line=2)
legend('topright',legend=c("Total","Males","Females"),lty=c(1,1,1),col=c("black","blue","red"))
dev.off()
